...
public class servletOrigen extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String _selectOrigen = "<option>-Selecciona el origen-</option>";
            ArrayList<Airport> _origins = new ArrayList();
            
            _origins = new Operation().airportOrigin(connection);
            
            for(int i=0; i<_origins.size() ; i++){
                _selectOrigen += "<option value="+(_origins.get(i)).getIdAirport()+">"+(_origins.get(i)).getCity()+"</option>";
            }
            session.setAttribute("airportsO", _origins);
            
            out.print(_selectOrigen);
